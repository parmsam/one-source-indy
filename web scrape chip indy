...
res_links <- scraping_hfh %>%  html_nodes("li") %>% str_extract('http://[\\w:[[:punct:]]]*/')
...
scrape_hfh <- function(link){
  ...
  rs_dataframe <- data.frame(rs_category, rs_names, rs_phones, rs_address, rs_hours, rs_required, rs_services) 
  return(rs_dataframe)
}
scrape_hfh("http://www.handbookofhelp.org/clothing-pantries/") %>% View()
full_dataset <- map(res_links, scrape_hfh)
list2env(full_dataset, envir=.GlobalEnv)
